...
class _SeriesEpisodeListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final episodes = List<Episode>.from(
      _view.episodes.map(
        (e) => e.copyWith(album: _view.series.title, image: _view.series.image),
      ),
    );
    return Builder(
      builder: (context) {
        ...
        return Column(
          children: [
            ...episodes.asMap().keys.toList().map(
              (index) => ListTile(
                isThreeLine: true,
                trailing: _trailing(
                  context,
  ...
  Widget _subtitle(
    ...
    return Builder(
      builder: (context) {
        ...
        children.add(
          RelativeDateWidget.from(episode.date, prefix: episode.author),
        );
        return Column(
...
class EpisodeListWidget extends StatelessWidget {
  ...
  void _onTapped(BuildContext context, Episode episode) {
    push(
      context,
      builder: (_) => _EpisodeWidget(episode, ''),
    ); // TODO need title
  }
}
