  ...
  Widget page(BuildContext context, TVSeriesView state) {
    ...
    return Scaffold(
        body: RefreshIndicator(
            ...
            child: RotaryList<TVEpisode>(state.episodes,
                title: state.series.name,
                subtitle: subtitle,
                tileBuilder: (context, entry) =>
                    tvEpisodeTile(context, entry, state))));
  }

  Widget tvEpisodeTile(
      BuildContext context, TVEpisode episode, TVSeriesView state) {
...
class _PageEntry {
  ...
  _PageEntry({this.icon, this.title, this.subtitle, this.onSelected});
}
...
class TVEpisodePage extends ClientPage<TVEpisodeView> {
  ...
  Widget page(BuildContext context, TVEpisodeView state) {
    ...
    return Scaffold(
        body: RefreshIndicator(
            onRefresh: () => reloadPage(context),
            child: RotaryList<_PageEntry>(entries,
                title: state.episode.name,
                subtitle: subtitle,
                tileBuilder: (context, entry) =>
                    _episodeTile(context, entry, state))));
  }

  Widget _episodeTile(
      BuildContext context, _PageEntry entry, TVEpisodeView state) {
    final enableStreaming = allowStreaming(context);
...
class _VideoPlayerPage extends StatelessWidget {
  ...
  void onPause(BuildContext context, Duration position, Duration duration) {
    context.updateProgress(state.episode.etag,
